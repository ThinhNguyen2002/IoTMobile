...
export class Hysen extends Device {
  ...
  private calculateCrc16(inputData: Buffer): number {
    const crc16Tab = [...Array(256).keys()].map(i =>
      [...(Array(8) as undefined[])].reduce(
        crc => (crc & 0x0001 ? (crc >> 1) ^ 0xa001 : crc >> 1),
        i,
      ),
    );
    return [...inputData].reduce(
      (crcValue, c) => (crcValue >> 8) ^ crc16Tab[(crcValue ^ c) & 0x00ff],
      0xffff,
    );
  }
        ...
        .then(response => {
          ...
          try {
            if (responsePayloadLength + 2 > responsePayload.length) {
              throw new Error(
                'hysenResponseError first byte of response is not length',
              );
            } else {
              const nom_crc = response.subarray(0, 2).readInt8();
              const real_crc = this.calculateCrc16(
                responsePayload.subarray(2, responsePayloadLength),
              );
              if (nom_crc !== real_crc) {
                resolve(responsePayload.subarray(2, responsePayloadLength));
              } else {
                throw new Error(
                  'hysenResponseError CRC check on response failed',
                );
              }
            }
          } catch (err) {
      ...
      this.sendRequest(Buffer.of(0x01, 0x03, 0x00, 0x00, 0x00, 0x16))
        .then(payload => {
          ...
          resolve({
            ...
            svl: payload[12],
            roomTempAdj:
              roomTempAdj > 32767 ? 32767 - roomTempAdj : roomTempAdj,
            fre: payload[15],
            poweron: payload[16],
            unknown: payload[17],
  ...
  public setTemp(temp: number): Promise<void> {
    return this.sendRequest(
      Buffer.of(0x01, 0x06, 0x00, 0x01, 0x00, temp * 2),
    ).then();
  }

  public setPower(power = 1, remoteLock = 0): Promise<void> {
    return this.sendRequest(
      Buffer.of(0x01, 0x06, 0x00, 0x00, remoteLock, power),
    ).then();
  }
  ...
  public setSchedule(weekday: DayModel[], weekend: DayModel[]): Promise<void> {
    return this.sendRequest(
      Buffer.from([
        ...
        0x18,
        ...[...Array(6).keys()]
          .map(i => [weekday[i].startHour, weekday[i].startMinute])
          .flat(),
        ...[...Array(2).keys()]
          .map(i => [weekend[i].startHour, weekend[i].startMinute])
          .flat(),
