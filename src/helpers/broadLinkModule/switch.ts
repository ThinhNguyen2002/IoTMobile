...
export class Sp2s extends Sp2 {
  ...
  public getEnergy(): Promise<number> {
    const packet = Buffer.alloc(16);
    packet[0] = 4;
    return this.sendPacketAndDecrypt(packet).then(
      payload => payload.subarray(0x4, 0x7).readInt8() / 1000,
    );
  }
}
...
export class Sp3s extends Sp2 {
  ...
  public getEnergy(): Promise<number> {
    return this.sendPacketAndDecrypt(
      Buffer.of(8, 0, 254, 1, 5, 1, 0, 0, 0, 45),
    ).then(payload => payload.subarray(0x7a, 0x4).readInt8() / 100);
  }
}
...
export class Sp4 extends Device {
  ...
  public setState(state: Partial<Sp4State>): Promise<Sp4State> {
    return this.sendPacket(this.encode(2, state)).then(payload =>
      this.decode(payload),
    );
  }
  ...
  public getState(): Promise<Sp4State> {
    return this.sendPacketAndDecrypt(this.encode(1, {})).then(payload =>
      this.decode(payload),
    );
  }
  ...
  protected getValue<I extends number | boolean, O extends number | boolean>(
    ...
    return {
      ...
      indicator:
        state.indicator !== undefined ? Number(state.indicator) : undefined,
      ntlbrightness: state.ntlbrightness,
      maxworktime: state.maxworktime,
      childlock:
        state.childlock !== undefined ? Number(state.childlock) : undefined,
    };
  }
  ...
  protected decode(payload: Buffer): Sp4State {
    return this.getValue(
      JSON.parse(
        payload
          .subarray(0x0c, 0x0c + struct('<I').unpack_from(payload, 0x08)[0])
          .toString(),
      ) as Sp4State<number>,
      Boolean,
    ) as Sp4State;
...
export class Sp4b extends Sp4 {
  ...
  protected encode(flag: number, state: Sp4State): Buffer {
    ...
    const checksum =
      packet.subarray(0x02).reduce((acc, b) => acc + b, 0xbeaf) & 0xffff;
  ...
  protected decode(payload: Buffer): Sp4State {
    const data = this.getValue(
      JSON.parse(
        payload
          .subarray(0x0e, 0x0e + struct('<I').unpack_from(payload, 0xa)[0])
          .toString(),
      ) as Sp4State,
      Boolean,
    ) as Sp4State;
    return {
      ...data,
      ntlbrightness:
        data.ntlbrightness !== undefined
          ? data.ntlbrightness / 1000
          : undefined,
      maxworktime:
        data.maxworktime !== undefined ? data.maxworktime / 1000 : undefined,
    } as Sp4State;
  }
}
...
export class Bg1 extends Device {
  ...
  private encode(flag: number, state: Partial<Bg1State>): Buffer {
    ...
    const checksum =
      packet.subarray(0x2).reduce((acc, b) => acc + b, 0xbeaf) & 0xffff;
    packet[0x06] = checksum & 0xff;
    packet[0x07] = (checksum >> 8) & 0xff;
    return packet;
  }
  ...
  private decode(payload: Buffer): Bg1State {
    return this.getValue(
      JSON.parse(
        payload
          .subarray(0x0e, 0x0e + struct('<I').unpack_from(payload, 0x0a)[0])
          .toString(),
      ) as Bg1State<number>,
      Boolean,
    ) as Bg1State;
