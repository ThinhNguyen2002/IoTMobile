...
export class Rmmini extends Device {
  ...
  protected send(command: number, data: number[] = []): Promise<Buffer> {
    return this.sendPacketAndDecrypt(
      Buffer.concat([struct('<I').pack(command), Buffer.from(data)]),
    ).then(payload => payload.subarray(0x4));
  }
...
export class Rmminib extends Rmmini {
  ...
  protected send(command: number, data: number[] = []): Promise<Buffer> {
    return this.sendPacketAndDecrypt(
      Buffer.concat([
        struct('<HI').pack(data.length + 4, command),
        Buffer.from(data),
      ]),
    ).then(payload => {
      return payload.subarray(
        0x6,
        struct('<H').unpack(payload.subarray(0, 0x2))[0] + 2,
      );
    });
  }
}
...
export class Rm4mini extends Rmminib {
  ...
  public checkSensors(): Promise<{temperature: number; humidity: number}> {
    return this.send(0x24).then(payload => {
      return {
        temperature:
          struct('<bb').unpack(payload.subarray(0, 0x2))[0x0] +
          payload[0x1] / 100.0,
